...
class DSSStructure {
    ...
    createDevice(dev, callback) {
        ...
        if (dev.hwInfo.startsWith('SW-') || dev.hwInfo.startsWith('IC SW-')) { // Joker / Tasten
            this.createJokerDevice(dev, devId, callback);
        }
        else if ((dev.hwInfo.startsWith('GE-') || dev.hwInfo.startsWith('IC GE-')) && !dev.hwInfo.includes('-UMV')) { // Yellow: Light
            this.createLightDevice(dev, devId, callback);
        }
        else if (dev.hwInfo.startsWith('GR-') || dev.hwInfo.startsWith('IC GR-')) { // Gray: Shades
            ...
            if (dev.outputMode && dev.outputChannels && dev.outputChannels.length) {
                if (dev.outputChannels.length === 1) {
                    ...
                    if (dssConstants.outputChannelUnitRoleMap[output.channelType] && dssConstants.outputChannelUnitRoleMap[output.channelType].native) {
                        ...
                        const channelIndex = roleDef.native.channelIndex !== undefined ? roleDef.native.channelIndex : output.channelIndex;
                        const nativeMax = roleDef.native.nativeMax;

                        if (output && this.adapter.config.initializeOutputValues) {
                            this.dssQueue.queueUpdateOutputValue(dev, channelIndex, nativeMax, 'medium', (err, value) => {
                                if (!err) {
                        ...
                        this.dssObjects[outputStateId].onChange = value => {
                            ...
                            if (nativeMax) {
                                ...
                                if (!dev.hwInfo.includes('-UMV') && (roleDef.max !== undefined && value === roleDef.min) || (roleDef.max !== undefined && value === roleDef.max)) {
                                    ...
                                    this.dssQueue.pushQueryQueue(dev.meterDSUID, {
                                        ...
                                        if (err || (res && !res.ok)) {
                                            ...
                                            this.adapter.setState(outputStateId, value ? roleDef.max : roleDef.min, true);
                                        }
                                    });
                                }
                                else {
                                    const normalizedValue = Math.round(value * nativeMax / roleDef.max);
                                    this.dssQueue.queueSetOutputValue(dev, channelIndex, nativeMax, normalizedValue, 'high', (err, _value)  => {
                                        if (err) {
                                ...
                                // send value directly
                                this.dssQueue.queueSetOutputValue(dev, channelIndex, 1, value, 'high', (err, _value)  => {
                                    if (err) {
                                        this.adapter.log.warn('Error when setting output value for ' + dev.dSUID + ': ' + err);
                                    }
                                    else {
    ...
    createApartment(apartment, reachableZoneGroups, callback) {
        ...
        if (aptStates && aptStates.length) {
            ...
            aptStates.forEach(state => {
                ...
                if (roleMap.native) {
                    if (roleMap.native.valueTrue !== undefined && stateValue == roleMap.native.valueTrue) {
                        stateValue = true;
                    } else if (roleMap.native.valueFalse !== undefined && stateValue == roleMap.native.valueFalse) {
                        stateValue = false;
                    }
                }
