    ...
    createDevice(dev, callback) {
        ...
        if (dev.binaryInputCount) {
            ...
            dev.binaryInputs.forEach((input, idx) => {
                if (!dssConstants.binaryInputTypeNames[input.inputType] && (input.inputType === 0 && !dssConstants.groupTypes[input.targetGroup])) {
                ...
                this.addStateObject(inputId, dev.dSUID + '.binaryInputs.' + idx, {
                    name: dssConstants.binaryInputTypeNames[input.inputType] || dssConstants.groupTypes[input.targetGroup] || 'Unknown',
                    type: 'boolean',
                    read: true,
                    write: false
                });
