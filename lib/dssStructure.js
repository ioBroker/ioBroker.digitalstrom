...
class DSSStructure {
        ...
        this.apartmentCircuits.forEach(circuit => {
            ...
            if (circuitStates && circuitStates.length) {
                ...
                circuitStates.forEach(state => {
                    ...
                    this.initialObjectValues[stateId] = state.value;
                });
            }
        });
    ...
    createDevice(dev, callback) {
        ...
        else if (dev.outputMode && dev.outputChannels && dev.outputChannels.length) {
            if (dev.outputChannels.length === 1) {
                ...
                if (output && this.adapter.config.initializeOutputValues) {
                    this.dssQueue.queueUpdateOutputValue(dev, dssConstants.outputChannelUnitRoleMap[output.channelType].native.channelIndex, dssConstants.outputChannelUnitRoleMap[output.channelType].native.nativeMax, 'medium', (err, value) => {
                        if (!err) {
                            if (dssConstants.outputChannelUnitRoleMap[output.channelType].native.nativeMax && dssConstants.outputChannelUnitRoleMap[output.channelType].common.max) {
                                value = Math.round(value * dssConstants.outputChannelUnitRoleMap[output.channelType].common.max / dssConstants.outputChannelUnitRoleMap[output.channelType].native.nativeMax);
                            }
                ...
                this.dssObjects[outputStateId].onChange = value => {
                    ...
                    if (dssConstants.outputChannelUnitRoleMap[output.channelType].native.nativeMax) {
                        if ((dssConstants.outputChannelUnitRoleMap[output.channelType].common.max !== undefined && value === dssConstants.outputChannelUnitRoleMap[output.channelType].common.min) || (dssConstants.outputChannelUnitRoleMap[output.channelType].common.max !== undefined && value === dssConstants.outputChannelUnitRoleMap[output.channelType].common.max)) {
                            value = (value > dssConstants.outputChannelUnitRoleMap[output.channelType].common.min);
                            this.dssQueue.pushQueryQueue(dev.meterDSUID, {
                                dssClass: 'device',
                                dssFunction: 'callScene',
                                params: {
                                ...
                                if (err || (res && !res.ok)) {
                                    ...
                                    this.adapter.setState(outputStateId, value ? dssConstants.outputChannelUnitRoleMap[output.channelType].common.max : dssConstants.outputChannelUnitRoleMap[output.channelType].common.min, true);
                                }
                            });
                        }
                        else {
                            const normalizedValue = Math.round(value * dssConstants.outputChannelUnitRoleMap[output.channelType].native.nativeMax / dssConstants.outputChannelUnitRoleMap[output.channelType].common.max);
    ...
    createApartment(apartment, reachableZoneGroups, callback) {
        ...
        if (aptStates && aptStates.length) {
            ...
            aptStates.forEach(state => {
                ...
                this.addStateObject(stateId, state.name, dssConstants.apartmentStateRoleMap[stateName]);
    ...
    processGroup(groupBaseId, zoneId, group, callback) {
        ...
        this.dss.requestAsync('zone', 'getReachableScenes' , {id: zoneId, groupID: group.id}).then((reachableScenes) => {
            ...
            if (reachableScenes.result.reachableScenes.length) {
                ...
                this.basicRoomScenes.forEach(sceneId => reachableScenes.result.reachableScenes.push(sceneId)); // enhance
