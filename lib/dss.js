...
class DSS extends EventEmitter {
        ...
        this.requestAsync('event', 'get', {subscriptionID: this.subscriptions[eventName].subscriptionId, timeout: this.subscriptions[eventName].timeout}).then( events => {
            ...
            if (events && events.result && events.result.events) {
                events.result.events.forEach(event => {
                    ...
                    if (this.subscriptions[eventName]) this.emit(event.name, event);
                });
            }
            this.pollEvent(eventName);
        }, (err) => {
            if (this.subscriptions[eventName]) {
                this.options.logger && this.options.logger.warn('Err: polling Event ' + eventName + ': ' + JSON.stringify(err));
                setTimeout(() => this.pollEvent(eventName), 5000);
            }
        });
    }
    ...
    unsubscribeEvent(eventName, callback) {
        this.requestAsync('event' , 'unsubscribe', {subscriptionID: this.subscriptions[eventName].subscriptionId, name: eventName}).then( (body) => {
            if (!body || !body.ok) {
