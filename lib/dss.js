...
class DSS extends EventEmitter {
    ...
    constructor(options) {
        ...
        this.subScriptionTimeout = options.subScriptionTimeout || 40 * 1000;
    ...
    pollEvent(eventName) {
        ...
        this.requestAsync('event', 'get', {subscriptionID: this.subscriptions[eventName].subscriptionId, timeout: this.subscriptions[eventName].timeout}).then( events => {
            ...
            if (this.subscriptions[eventName]) {
                ...
                let delay = 5000;
                if (this.subscriptions[eventName].errorCount > 2 || err.status_code === 500) {
                    this.subscribeEvent(eventName, this.subscriptions[eventName].subscriptionId, this.subscriptions[eventName].timeout, err => {
                        if (err) {
                            this.options.logger && this.options.logger.warn('Error on resubscribing to ' + eventName + ': ' + err);
                            this.emit('eventError', eventName, this.subscriptions[eventName].errorCount, 'Error on resubscribing to ' + eventName + ': ' + err);
                        }
                        else {
                            this.subscriptions[eventName].errorCount = 0;
    ...
    subscribeEvent(eventName, subscriptionId, timeout, callback) {
        this.requestAsync('event' , 'subscribe', {subscriptionID: subscriptionId, name: eventName}).then( (body) => {
            ...
            this.subscriptions[eventName] = this.subscriptions[eventName] || {subscriptionId, timeout};
            this.pollEvent(eventName);
