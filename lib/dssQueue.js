class DSSQueue {
    ...
    pushQueryQueue(circuit, entryId, entry, prio, callback) {
        if (typeof entryId !== 'string') {
            ...
            prio = entry;
            entry = entryId;
            entryId = entry.dssClass + '-' + entry.dssFunction + '-' + entry.params.dsuid + (entry.params.index || entry.params.offset || entry.params.sceneNumber);
        }
        prio = prio || 'low';
        ...
        if (prio !== 'low' && nextProcessIndex !== -1 && this.queryQueue[circuit][nextProcessIndex].prio === 'low') {
            // do not wait 5s, need to be faster
            this.nextEntryTimeout[circuit] && clearTimeout(this.nextEntryTimeout[circuit]);
            this.nextEntryTimeout[circuit] = null;
        }

        if (this.queryQueue[circuit].length === 1 && !this.nextEntryTimeout[circuit] && found === -1) {
    ...
    processQueryQueue(circuit) {
        ...
        this.dss.requestAsync(query.entry.dssClass, query.entry.dssFunction, query.entry.params).then((res) => {
            ...
            console.log(Date.now() + ' Queued Query processed ' + query.entry.dssClass + '/' + query.entry.dssFunction + ' and ' + JSON.stringify(query.entry.params) + ': ' + JSON.stringify(res));
            ...
            this.nextEntryTimeout[circuit] = setTimeout(() => {
                this.nextEntryTimeout[circuit] = null;
                this.processQueryQueue(circuit);
            }, (nextProcessIndex !== -1 && this.queryQueue[circuit][nextProcessIndex].prio !== 'low') ? 1000 : 5000);
        }, (err) => {
            const query = this.queryQueue[circuit].splice(toProcess, 1)[0];

            console.log('Queued Query ERROR ' + query.entry.dssClass + '/' + query.entry.dssFunction + ' and ' + JSON.stringify(query.entry.params) + ': ' + JSON.stringify(err));
