class DSSQueue {
    ...
    pushQueryQueue(circuit, entryId, entry, prio, callback) {
        if (typeof entryId !== 'string') {
            ...
            prio = entry;
            entry = entryId;
            entryId = entry.dssClass + '-' + entry.dssFunction + '-' + entry.params.dsuid + '-' + entry.params.id + '-' + entry.params.groupID + '-' + (entry.params.index || entry.params.offset || entry.params.sceneNumber);
        }
        prio = prio || 'low';
