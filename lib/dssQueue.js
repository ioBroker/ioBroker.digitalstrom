class DSSQueue {
    ...
    pushQueryQueue(circuit, entryId, entry, prio, callback) {
        ...
        if (nextProcessIndex !== -1) {
            ...
            this.options.logger && this.options.logger.debug(Date.now() + ' PUSH Plan next queue entry (' + entryId + ') for ' + circuit + ' and prio ' + this.queryQueue[circuit][nextProcessIndex].prio + ' in ' + nextEntryTimeout + 'ms');
